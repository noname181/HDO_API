...
module.exports = {
  path: ['/mobile/terms/read/require'],
  method: 'POST',
  checkToken: true,
  roles: [],
  service: service,
...
async function service(_request, _response, next) {
  ...
  try {
    ...
    for (let term of body.termIds) {
      if (!term.hasOwnProperty('termId') || !term.hasOwnProperty('target') || !term.hasOwnProperty('targetId')) {
...
function errorHandler(_error, _request, _response, next) {
  ...
  if (_error === 'NO_REQUIRED_INPUT') {
    _response.error.notFound(_error, '필수 입력 정보가 누락되었습니다.(target, targetId, termIds, category)');
    return;
  }
