...
async function service(_request, _response, next) {
  ...
  const { count: totalCount, rows: files } = await models.FileToCharger.findAndCountAll({
    where: { division: target, newestVersion: true },
    attributes: {
      exclude: ['deletedAt'],
    },
    limit: 1,
  ...
  if (update === 'NEED') {
    if (target === 'QR') {
      ...
      where.adTransDate = { [Op.eq]: null };
    } else if (target === 'TM') {
      where.tmTransDate = { [Op.eq]: null };
    } else if (target === 'FW') {
    } else {
      throw 'NOT_EXIST_CATEGORY';
    }
    ...
    if (target === 'QR') {
      ...
      where.adTransDate = { [Op.ne]: null };
    } else if (target === 'TM') {
      where.tmTransDate = { [Op.ne]: null };
    } else if (target === 'FW') {
    } else {
      throw 'NOT_EXIST_CATEGORY';
    }
  ...
  try {
    ...
    const { count: totalCount, rows: chargersData } = await models.sb_charger.findAndCountAll({...options, subQuery: false});
    ...
    // console.log("chargers lenght::", chargers.length);
    // const sss = [...new Set(chargers)]
    // console.log("sss length::", sss.length);
