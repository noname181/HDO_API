...
const { payRequestFromKICC, refundRequestFromKICC, refundALLRequestFromKICC, createTransactionID, formatDate, getMsgAuthValue } = require("../../util/paymentUtil")
...
const { sendRefundMsg, sendComChgMsg, sendComChgMsgMem, sendChgHisMsg, sendPartCancelFailedMsg  } = require("../../util/notificationTalk/notificationTemplate")
  ...
  try {
    ...
    if (old_status === "charging" && new_status === "finishing") {
      ...
      if (clog?.cl_order_user_no.startsWith("IC")) {
        ...
        await sendTalkAndLms(clog?.receivePhoneNo, sendComeChgMsgData?.message, sendComeChgMsgData?.templateCode, clog?.chg_id);
      } else {
        const sendComeChgMsgMemData = sendComChgMsgMem(dateString, originalKwhToFixed, appliedPrice, feeString);
        await sendTalkAndLms(clog?.receivePhoneNo, sendComeChgMsgMemData?.message, sendComeChgMsgMemData?.templateCode, clog?.chg_id);
      }
    }

    // 계산되는 시점
    if ((old_status==="finishing" && new_status==="available") || (isStopMeterExist && new_status==="available")) {
      ...
      if (clog?.cl_order_user_no.startsWith("IC")) {
        ...
        if (refundAmt === 0) {
          ...
          const sendChgHisMsgZero = sendChgHisMsg(prePaidChargeString, adjustedKwhString, clog?.appliedUnitPrice, chargeFeeString, unplugDateString, refundAmtString, actualPaidAmtString, phoneString)
          ...
          // Todo 문자 전송하기
          await sendTalkAndLms(clog?.receivePhoneNo, sendChgHisMsgZero?.message, sendChgHisMsgZero?.templateCode, clog?.chg_id);
          ...
          if (isRefundRequestSuccess) {
            ...
            const sendChgHisMsgNormal = sendChgHisMsg(prePaidChargeString, usedKwhString, clog?.appliedUnitPrice, chargeFeeString, unplugDateString, refundAmtString, actualPaidAmtString, phoneString)
            ...
            // Todo 문자 전송하기
            await sendTalkAndLms(clog?.receivePhoneNo, sendChgHisMsgNormal?.message, sendChgHisMsgNormal?.templateCode, clog?.chg_id);
            msg = "성공적으로 결제 취소를 했습니다."
          } else {
            const sendPartCancelFailedData = sendPartCancelFailedMsg(actualPaidAmtString, phoneString)
            await sendTalkAndLms(clog?.receivePhoneNo, sendPartCancelFailedData?.message, sendPartCancelFailedData?.templateCode, clog?.chg_id);
        ...
        */
        ...
        if (!isPayRequestSuccess) {
          ...
          const sendChgHisMsgNormal = sendChgHisMsg('0', usedKwhString, clog?.appliedUnitPrice, chargeFeeString, unplugDateString, '0', chargeFeeString, phoneString)
          await sendTalkAndLms(clog?.receivePhoneNo, sendChgHisMsgNormal?.message, sendChgHisMsgNormal?.templateCode, clog?.chg_id);
        }
      }
    }
    ...
    _response.json({result: `신호정상수신 ${old_status} => ${new_status}
