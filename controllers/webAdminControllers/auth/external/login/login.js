...
const { passwordValidator, accountIdAdminValidator, emailValidator } = require('../../../../../util/validators');
...
async function service(request, response, next) {
  ...
  try {
    ...
    const requestInfo = `, ip: ${requestIp.getClientIp(
      request
    )}, url: ${originalUrl},  method: ${method}, user-agent: ${headers['user-agent']}`;
...
function validator(request, response, next) {
  ...
  const isValidAccountId = emailValidator(accountId);
  if (!isValidAccountId) {
...
function errorHandler(error, request, response, next) {
  ...
  if (error === 'ACCOUNT_ID_IS_EMPTY') {
    return response.error.badRequest(error, '이메일을 입력해주세요.');
  }
